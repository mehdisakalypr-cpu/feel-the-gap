...
export async function GET(req: NextRequest) {
  ...
  const { count: opps }      = await admin.from('opportunities').select('*', { count: 'exact', head: true })

  return NextResponse.json({ countries, products, opportunities: opps, seed_available: SEED_TRADE_DATA.length })
}
...
export async function POST(req: NextRequest) {
  ...
  if (action === 'reseed_countries') {
    const countryRows = SEED_TRADE_DATA
      .filter(c => META[c.iso3])
      .map(c => {
        const m = META[c.iso3]
        return {
          id: c.iso3, iso2: m.iso2, name: m.name, name_fr: c.name_fr, flag: m.flag,
          region: m.region, sub_region: m.sub_region, lat: m.lat, lng: m.lng,
          population: m.population ?? null, gdp_usd: m.gdp_usd ?? null,
          gdp_per_capita: m.population && m.gdp_usd ? Math.round(m.gdp_usd / m.population) : null,
          arable_land_pct: c.arable_land_pct, total_imports_usd: c.total_imports_usd,
          total_exports_usd: c.total_exports_usd, trade_balance_usd: c.total_exports_usd - c.total_imports_usd,
          top_import_category: c.top_categories[0] ?? null, data_year: c.data_year,
        }
      })
    const { error } = await admin.from('countries').upsert(countryRows, { onConflict: 'id' })
    if (error) return NextResponse.json({ error: error.message }, { status: 500 })
    return NextResponse.json({ ok: true, count: countryRows.length, action })
  }

  if (action === 'reseed_products') {
    ...
    for (const c of SEED_TRADE_DATA) {
      for (const imp of c.top_imports ?? []) {
        ...
        if (!productMap.has(id)) {
          productMap.set(id, { id, hs2: imp.hs2.padStart(2, '0'), hs4: imp.hs2.padStart(4, '0'),
            name: imp.product, name_fr: imp.product, category: imp.category, subcategory: imp.category, unit: 'tonnes' })
        }
      }
    }
    const productRows = Array.from(productMap.values())
    const { error } = await admin.from('products').upsert(productRows, { onConflict: 'id' })
    if (error) return NextResponse.json({ error: error.message }, { status: 500 })
    return NextResponse.json({ ok: true, count: productRows.length, action })
  }

  if (action === 'reseed_opportunities') {
    ...
    for (const c of SEED_TRADE_DATA) {
      ...
      for (const gap of c.key_gaps ?? []) {
        const matched = productList.find((p: { id: string; name: string }) =>
          p.name.toLowerCase().includes(gap.product.split(' ')[0].toLowerCase()))
        if (!matched) continue
        oppRows.push({
          country_iso: c.iso3, product_id: matched.id, type: gap.opportunity_type,
          gap_value_usd: gap.gap_value_usd, opportunity_score: Math.min(100, Math.max(0, gap.score)),
    ...
    const { error } = await admin.from('opportunities').insert(oppRows)
    if (error && !error.message.includes('duplicate')) {
      return NextResponse.json({ error: error.message }, { status: 500 })
    }
    return NextResponse.json({ ok: true, count: oppRows.length, action })
  }

  if (action === 'clear_opportunities') {
    const { error } = await admin.from('opportunities').delete().neq('id', '00000000-0000-0000-0000-000000000000')
    if (error) return NextResponse.json({ error: error.message }, { status: 500 })
