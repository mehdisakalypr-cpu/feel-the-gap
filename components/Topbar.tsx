...
export default function Topbar() {
  ...
  return (
    <header className="h-14 flex items-center justify-between px-4 border-b border-[rgba(201,168,76,.15)] bg-[#0D1117] shrink-0 z-50 overflow-x-auto overflow-y-hidden scrollbar-hide gap-2">
      {/* Brand */}
      ...
      {/* Search */}
      <div className="relative w-72 min-w-[180px] shrink-0">
      ...
      <nav className="flex items-center gap-1 text-sm shrink-0">
        ...
        <Link href="/gemini" className="px-3 py-1.5 text-gray-400 hover:text-white rounded-md hover:bg-white/5 transition-colors flex items-center gap-1.5">
        ...
        {userInitial ? (
          <Link href="/account" className="ml-2 flex items-center gap-2 group">
            {tier && tier !== 'free' && (
              <span
